...
// Some stuff.
$url = new moodle_url('/blocks/analytics/report.php', array('courseid' => $courseid));
if ($action) {
    $url->param('action', $action);
}
$strcoursereport = get_string('coursereport', 'block_analytics');
...
if ($resetdata && confirm_sesskey()) {
    if ($confirm) {
        ...
        $resetstr = empty($group) ? get_string('reallyresetdata', 'block_analytics') : get_string('reallyresetgroupdata', 'block_analytics');
...
if ($action == 'edit' && !empty($userid)) {
    ...
    $form = new block_analytics_user_edit_form($url->out(false));
    $form->set_data(array('userid' => $userid, 'level' => $progress->level, 'analytics' => $progress->analytics));

    if ($data = $form->get_data()) {
        $manager->reset_user_analytics($userid, $data->analytics);
...
$table = new block_analytics_report_table('block_analytics_report', $courseid, $group);
$table->define_baseurl($url);

echo $table->out(20, true);

if (empty($group)) {
    $strreset = get_string('resetcoursedata', 'block_analytics');
} else {
    $strreset = get_string('resetgroupdata', 'block_analytics');
}
