...
// Some stuff.
$url = new moodle_url('/blocks/analytics/visuals.php', array('courseid' => $courseid));
$strcoursevisuals = get_string('coursevisuals', 'block_analytics');
...
file_prepare_draft_area($draftitemid, $context->id, 'block_analytics', 'badges', 0, $fmoptions);

$data = new stdClass();
$data->badges = $draftitemid;
$data->enablecustomlevelbadges = $manager->get_config('enablecustomlevelbadges');

$form = new block_analytics_visuals_form($url->out(false), array('manager' => $manager, 'fmoptions' => $fmoptions));
$form->set_data($data);

if ($data = $form->get_data()) {
    file_save_draft_area_files($data->badges, $context->id, 'block_analytics', 'badges', 0, $fmoptions);
