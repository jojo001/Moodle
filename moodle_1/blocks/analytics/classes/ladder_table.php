 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_analytics_ladder_table extends table_sql {
    ...
    protected $identitymode = block_analytics_manager::IDENTITY_ON;
    ...
    protected $rankmode = block_analytics_manager::RANK_ON;
    ...
    protected $startinganalytics;

    /** @var int The XP to compare with. Used with RANK_REL. */
    protected $startinganalyticsdiff;

    /** @var array The fields found in the XP table. */
    public static $analyticsfields = array('id', 'courseid', 'userid', 'analytics', 'lvl');
    ...
    public function __construct($uniqueid, $courseid, $groupid, array $options = array(), $userid = null) {
        ...
        $columns = array();
        $headers = array();
        if ($this->rankmode != block_analytics_manager::RANK_OFF) {
            $columns += array('rank');
            if ($this->rankmode == block_analytics_manager::RANK_REL) {
                $headers += array(get_string('difference', 'block_analytics'));
            } else {
                $headers += array(get_string('rank', 'block_analytics'));
            }
        }
        ...
        if ($groupid) {
            ...
            $sqlfrom = '{block_analytics} x JOIN {user} u ON x.userid = u.id';
        }
    ...
    function build_table() {
        ...
        $lastanalytics = $this->startinganalytics;
        $offset = $this->startingoffset;
        $analyticstodiff = $this->startinganalyticsdiff;
        ...
        if ($this->rawdata) {
            foreach ($this->rawdata as $row) {
                ...
                // Show the real rank.
                if ($this->rankmode == block_analytics_manager::RANK_ON) {

                    // If this row is different than the previous one.
                    if ($row->lvl != $lastlvl || $row->analytics != $lastanalytics) {
                        $rank += $offset;
                        $offset = 1;
                        $lastlvl = $row->lvl;
                        $lastanalytics = $row->analytics;
                    } else {
                        $offset++;
                    }
                    $row->rank = $rank;
                ...
                } else if ($this->rankmode == block_analytics_manager::RANK_REL) {

                    // There was no indication of what XP to diff with, let's take the first entry.
                    if ($analyticstodiff == -1 && $lastanalytics == -1) {
                        $analyticstodiff = $row->analytics;
                    }

                    // The last row does not this one.
                    if ($row->analytics != $lastanalytics) {
                        $rank = $row->analytics - $analyticstodiff;
                        $lastanalytics = $row->analytics;
                    }
    ...
    public function col_fullname($row) {
        if ($this->identitymode == block_analytics_manager::IDENTITY_OFF && $row->userid != $this->userid) {
            return get_string('someoneelse', 'block_analytics');
        }
    ...
    protected function col_progress($row) {
        static $fields = null;
        if ($fields === null) {
            $fields = array_flip(self::$analyticsfields);
        }

        $record = (object) array_intersect_key((array) $row, $fields);
        $progress = $this->analyticsmanager->get_progress_for_user($row->userid, $record);
    ...
    protected function col_rank($row) {
        if ($this->rankmode == block_analytics_manager::RANK_REL && $row->rank > 0) {
            return '+' . $row->rank;
        }
        return $row->rank;
    }
    ...
    protected function col_userpic($row) {
        ...
        if ($this->identitymode == block_analytics_manager::IDENTITY_OFF && $this->userid != $row->userid) {
    ...
    protected function compute_rank_start() {
        ...
        $this->startinganalytics = -1;
        $this->startingoffset = 1;
        $this->startinganalyticsdiff = -1;

        // Guess the starting rank.
        if ($this->rankmode == block_analytics_manager::RANK_ON && !empty($this->rawdata)) {
            $record = reset($this->rawdata);
            $sql = "SELECT COUNT(x.id)
                      FROM {$this->sql->from}
                     WHERE {$this->sql->where}
                       AND x.analytics > :neighanalytics";
            $this->startingrank = $DB->count_records_sql($sql, $this->sql->params + array('neighanalytics' => $record->analytics)) + 1;
            $params = $this->sql->params + array(
                'neighid' => $record->id,
                'neighanalytics' => $record->analytics,
                'neighanalyticseq' => $record->analytics
            );
            $sql = "SELECT COUNT(x.id)
                      FROM {$this->sql->from}
                     WHERE {$this->sql->where}
                       AND (x.analytics > :neighanalytics
                        OR (x.analytics = :neighanalyticseq AND x.id < :neighid))";
            $this->startingoffset = 1 + $DB->count_records_sql($sql, $params) - $this->startingrank;
            $this->startinglevel = $record->lvl;
            $this->startinganalytics = $record->analytics;
        ...
        // When relative, set self XP as difference.
            ...
            if ($record) {
                $this->startinganalyticsdiff = $record->analytics;
            }
        }
    }
    ...
    protected function get_user_record() {
        ...
        if ($this->currentuserrecord === null) {
            ...
            if (empty($record) && $this->neighboursonly && $this->analyticsmanager->can_manage()) {
                $record = (object) array(
                    'id' => 0,
                    'userid' => $this->userid,
    ...
    public function get_sort_columns() {
        return array('x.lvl' => SORT_DESC, 'x.analytics' => SORT_DESC, 'x.id' => SORT_ASC);
    }
        ...
        if (!$this->is_downloading()) {
            ...
            if ($requestedpage === null && ($record = $this->get_user_record())) {
                $sql = "SELECT COUNT('x')
                          FROM {$this->sql->from}
                         WHERE {$this->sql->where}
                           AND (x.analytics > :theanalytics
                            OR (x.analytics = :theanalyticseq AND x.id < :theid))";
                $params = $this->sql->params + array(
                    'theanalytics' => $record->analytics,
                    'theanalyticseq' => $record->analytics,
                    'theid' => $record->id
                );
    ...
    public function query_db_neighbours($userid, $abovecount, $belowcount) {
        ...
        $params = $this->sql->params + array(
            'neighid' => $me->id,
            'neighanalytics' => $me->analytics,
            'neighanalyticseq' => $me->analytics,
        );
        $sqlabove = "SELECT {$this->sql->fields}
                      ...
                      WHERE {$this->sql->where}
                        AND (x.analytics > :neighanalytics
                         OR (x.analytics = :neighanalyticseq AND x.id < :neighid))
                      ORDER BY x.analytics ASC, x.id DESC";
        $sqlbelow = "SELECT {$this->sql->fields}
                       FROM {$this->sql->from}
                      WHERE {$this->sql->where}
                        AND (x.analytics < :neighanalytics
                         OR (x.analytics = :neighanalyticseq AND x.id > :neighid))
                      ORDER BY x.analytics DESC, x.id ASC";
