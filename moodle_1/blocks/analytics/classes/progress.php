...
class block_analytics_progress implements renderable {
    ...
    public $analyticsforlevel = null;

    /** @var int The XP in the current level, relative to the current level. */
    public $analyticsinlevel = null;

    /** @var array The required fields. */
    protected static $required = array('courseid', 'contextid', 'userid', 'analytics', 'level', 'levelanalytics', 'nextlevelanalytics');
    ...
    public function __construct(array $params) {
        ...
        if (count($diff) > 0) {
            throw new coding_exception('Missing, or uneanalyticsected, properties');
        }
        ...
        // Has next level.
        if (!empty($this->nextlevelanalytics) && $this->nextlevelanalytics > $this->analytics) {
            $this->hasnextlevel = true;
            $this->nextlevel = $this->level + 1;

            // Percentage.
            $this->analyticsinlevel = ($this->analytics - $this->levelanalytics);
            $this->analyticsforlevel = ($this->nextlevelanalytics - $this->levelanalytics);
            $this->percentage = round($this->analyticsinlevel / $this->analyticsforlevel * 100);
        } else {
            $this->nextlevel = null;
            $this->hasnextlevel = true;
            $this->analyticsforlevel = $this->analytics;
            $this->analyticsinlevel = $this->analytics;
            $this->percentage = 100;
        }

        // Image URL.
        $this->levelimgsrc = moodle_url::make_pluginfile_url($this->contextid, 'block_analytics',
            'badges', 0, '/', $this->level);
