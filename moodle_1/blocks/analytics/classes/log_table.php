 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_analytics_log_table extends table_sql {
    ...
    public function __construct($uniqueid, $courseid, $groupid) {
        ...
        $this->define_headers(array(
            get_string('eventtime', 'block_analytics'),
            get_string('fullname'),
            get_string('analytics', 'block_analytics'),
            get_string('eventname', 'block_analytics')
        ));
        ...
        if ($groupid) {
            ...
            $sqlfrom = '{block_analytics_log} x LEFT JOIN {user} u ON x.userid = u.id';
        }
