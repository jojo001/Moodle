 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_analytics_rule_event extends block_analytics_rule_property {
    ...
    public function get_description() {
        $class = $this->value;
        $name = get_string('errorunknownevent', 'block_analytics');
        $infos = self::get_event_infos($class);

        if ($infos !== false) {
            $pluginmanager = core_plugin_manager::instance();
            $plugininfo = $pluginmanager->get_plugin_info($infos['component']);
            $name = get_string('colon', 'block_analytics', (object) array(
                'a' => $plugininfo->displayname,
                'b' => $infos['name']
            ));
        }

        return get_string('ruleeventdesc', 'block_analytics', (object)array('eventname' => $name));
    }
    ...
    public static function get_events_list() {
        global $CFG;
        $cache = cache::make('block_analytics', 'ruleevent_eventslist');
        $key = 'list';
            ...
            foreach ($pluginlist as $plugin => $directory) {
                ...
                foreach ($finalfiles as $eventname => $notused) {
                    ...
                    if ($infos !== false && $infos['edulevel'] == \core\event\base::LEVEL_PARTICIPATING) {
                        $events[$infos['eventname']] = get_string('colon', 'block_analytics', (object) array(
                            'a' => $plugininfo->displayname,
                            'b' => $infos['name']
                        ));
                    }
