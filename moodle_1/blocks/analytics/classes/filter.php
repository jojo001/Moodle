 ...
 * Filter class.
 *
 * The filter only works with block_analytics_rule_property, or rulesets containing them.
 *
 * @package    block_analytics
 * @copyright  2014 Frédéric Massart - FMCorz.net
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_analytics_filter implements renderable {
    ...
    public function delete() {
        ...
        $DB->delete_records('block_analytics_filters', array('id' => $this->id));
    }
     ...
     * @return block_analytics_filter The filter.
     */
    ...
    protected function load_rule() {
        ...
        $this->rule = block_analytics_rule::create($ruledata);
    }
    ...
    public function save() {
        ...
        if (!$this->id) {
            $this->id = $DB->insert_record('block_analytics_filters', $record);
        } else {
            $record->id = $this->id;
            $DB->update_record('block_analytics_filters', $record);
        }
    }
     ...
     *
     * @param block_analytics_rule $rule
     */
    public function set_rule(block_analytics_rule $rule) {
        $this->rule = $rule;
        $this->ruledata = json_encode($rule->eanalyticsort());
    }
