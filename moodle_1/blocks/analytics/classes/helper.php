...
class block_analytics_helper {
    ...
    public static function course_deleted(\core\event\course_deleted $event) {
        ...
        $DB->delete_records('block_analytics', $conditions);
        $DB->delete_records('block_analytics_config', $conditions);
        $DB->delete_records('block_analytics_filters', $conditions);
        $DB->delete_records('block_analytics_log', $conditions);

        // Delete the files.
        $fs = get_file_storage();
        $fs->delete_area_files($event->contextid, 'block_analytics', 'badges');
    }
    ...
    public static function observer(\core\event\base $event) {
        ...
        if ($event->component === 'block_analytics') {
            // Skip own events.
        ...
        } else if (!has_capability('block/analytics:earnanalytics', $event->get_context(), $event->userid)) {
            // Skip the events if the user does not have the capability to earn XP.
        } else {
            // Keep the event, and proceed.
            $manager = block_analytics_manager::get($event->courseid);
