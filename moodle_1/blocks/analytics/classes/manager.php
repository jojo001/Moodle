...
class block_analytics_manager {
    ...
    const USERPREF_NOTIFY = 'block_analytics_notify_level_up_';

    /** User preference key storing if they dismissed the like notice */
    const USERPREF_NOTICES = 'block_analytics_notices';
    ...
    /** @var block_analytics_filter_manager Cache of the manager. */
    ...
    protected function __construct($courseid) {
        ...
        if ($CFG->block_analytics_context == CONTEXT_SYSTEM && $courseid != SITEID) {
            throw new coding_exception('Uneanalyticsected course ID.');
        }
     ...
     * 200x times to get more eanalyticserience points. For simplicity, and performance
    ...
    protected function can_capture_event(\core\event\base $event) {
        ...
        if (!isset($SESSION->block_analytics_cheatguard)) {
            $SESSION->block_analytics_cheatguard = array();
        }

        // Actions per time.
        if (count($SESSION->block_analytics_cheatguard) > $maxactions) {
            $actions = array_reverse($SESSION->block_analytics_cheatguard, true);
            $count = 0;
        ...
        if (isset($SESSION->block_analytics_cheatguard[$actionkey])) {
            if ($SESSION->block_analytics_cheatguard[$actionkey] > $now - $actiontime) {
                // The key was found and the time has not eanalyticsired, cheater spotted.
                return false;
            }
        }

        // Unset the value to re-add it at the end of the array.
        unset($SESSION->block_analytics_cheatguard[$actionkey]);

        // Log the time at which this event happened.
        $SESSION->block_analytics_cheatguard[$actionkey] = time();

        // Limit the array of events to $maxcount, we do not want to flood the session for no reason.
        $SESSION->block_analytics_cheatguard = array_slice($SESSION->block_analytics_cheatguard, -$maxcount, null, true);
        return true;
    }
    ...
    public function can_manage() {
        return has_capability('block/analytics:addinstance', $this->context);
    }
    ...
    public function can_view() {
        return has_capability('block/analytics:view', $this->context) || $this->can_manage();
    }
    ...
    public function capture_event(\core\event\base $event) {
        ...
        if ($CFG->block_analytics_context != CONTEXT_SYSTEM && $event->courseid != $this->courseid) {
        ...
        $userid = $event->userid;
        $points = $this->get_analytics_from_event($event);

        // No need to go through the following if the user did not gain XP.
        if ($points > 0) {
            if ($DB->count_records('block_analytics', array('courseid' => $this->courseid, 'userid' => $userid)) > 0) {
                $DB->execute('UPDATE {block_analytics} SET analytics = analytics + :analytics WHERE courseid = :courseid AND userid = :userid',
                    array('analytics' => $points, 'courseid' => $this->courseid, 'userid' => $userid));
            } else {
                $record = new stdClass();
                $record->courseid = $this->courseid;
                $record->userid = $userid;
                $record->analytics = $points;
                $DB->insert_record('block_analytics', $record);
            }
     ...
     * @return block_analytics_manager The instance of the manager.
     */
    public static function get($courseid, $forcereload = false) {
        ...
        if ($CFG->block_analytics_context == CONTEXT_SYSTEM) {
            $courseid = SITEID;
        }

        $courseid = intval($courseid);
        if ($forcereload || !isset(self::$instances[$courseid])) {
            self::$instances[$courseid] = new block_analytics_manager($courseid);
        }
    ...
    public function get_config($name = null) {
        ...
        if (empty($this->config)) {
            $record = $DB->get_record('block_analytics_config', array('courseid' => $this->courseid));
            if (!$record) {
     ...
     * courses ($CFG->block_analytics_context == CONTEXT_COURSE), then the context of the manager
     * should be the course context. However, when block_analytics_progress is set to CONTEXT_SYSTEM
     ...
     *
     * When the block is set to act on the whole site ($CFG->block_analytics_context == CONTEXT_SYSTEM),
     ...
     *
     * @return block_analytics_filter_manager
     */
    public function get_filter_manager() {
        if (!$this->filtermanager) {
            $this->filtermanager = new block_analytics_filter_manager($this);
        }
    ...
    /**
     * Return the level at which we are at $analytics.
     *
     * @param int $analytics XP acquired.
     * @return int The level.
     */
    public function get_level_from_analytics($analytics) {
        $levels = $this->get_levels();

        $level = 1;
        for ($i = $level; $i <= count($levels); $i++) {
            if ($levels[$i] <= $analytics) {
                $level = $i;
            } else {
                break;
            }
    ...
    /**
     * Get the levels and the eanalyticserience points needed.
     *
     * @return array level => analytics required.
     */
    public function get_levels() {
        if (empty($this->levels)) {
            $eventsdata = $this->get_levels_data('levelsdata');
            $this->levels = $eventsdata['analytics'];
        }
        return $this->levels;
    }
    ...
    public function get_levels_data() {
        ...
        return array(
            'usealgo' => 1,
            'base' => block_analytics_manager::DEFAULT_BASE,
            'coef' => block_analytics_manager::DEFAULT_COEF,
            'analytics' => self::get_levels_with_algo($this->get_level_count()),
            'desc' => array()
        );
    }
     ...
     * @return block_analytics_progress The progress renderable.
     */
    public function get_progress_for_user($userid, stdClass $record = null) {
        global $DB;

        if (!$record) {
            $record = $DB->get_record('block_analytics', array('courseid' => $this->courseid, 'userid' => $userid));
        }
        ...
        $params['levelanalytics'] = $this->get_analytics_for_level($record->level);
        $params['nextlevelanalytics'] = $this->get_analytics_for_level($record->level + 1);
        $progress = new block_analytics_progress($params);

        return $progress;
    }

    /**
     * Get the eanalyticserience points generated by an event.
     *
     * @param \core\event\base $event The event.
     * @return int XP points.
     */
    public function get_analytics_from_event(\core\event\base $event) {
     ...
     */
    public function get_analytics_for_level($level) {
        $levels = $this->get_levels();
     ...
     * @param int $userid The user ID.
     * @param int $analytics The XP earned with that event.
     * @return void
     */
    protected function log_event($eventname, $userid, $analytics) {
        global $DB;
        ...
        if ($this->get_config('enablelog')) {
            ...
            $record->time = time();
            try {
                $DB->insert_record('block_analytics_log', $record);
    ...
    public function purge_log() {
        ...
        if (!$keeplogs) {
            ...
            $DB->delete_records_select('block_analytics_log', 'time < :time', array(
    ...
    public function recalculate_levels() {
        global $DB;
        $users = $DB->get_recordset('block_analytics', array('courseid' => $this->courseid), '', 'userid, lvl, analytics');
        ...
        foreach ($users as $user) {
            $this->update_user_level($user->userid, $user->analytics, $user->lvl);
        }
    ...
    public function reset_data($groupid = 0) {
        ...
        // Delete XP records.
        $sql = "DELETE FROM {block_analytics} WHERE courseid = :courseid";
        ...
        // Delete logs.
        $sql = "DELETE FROM {block_analytics_log} WHERE courseid = :courseid";
        $params = array('courseid' => $this->courseid);
        if ($groupid) {
            $sql .= " AND userid IN
                  (SELECT gm.userid
     ...
     *
     * @param int $userid The user ID.
     * @param int $analytics The amount of XP.
     * @return void
     */
    public function reset_user_analytics($userid, $analytics = 0) {
        global $DB;

        if ($record = $DB->get_record('block_analytics', array('courseid' => $this->courseid, 'userid' => $userid))) {
            $record->analytics = $analytics;
            $DB->update_record('block_analytics', $record);
        } else {
            $record = new stdClass();
            $record->courseid = $this->courseid;
            $record->userid = $userid;
        ...
        $oldtriggerevents = $this->triggereevents;
        $this->triggereevents = false;
        $this->update_user_level($userid, $record->analytics, $record->lvl);
        $this->triggereevents = $oldtriggerevents;
    }
    ...
    public function update_config($data) {
        ...
        if (empty($config->id)) {
            $config->id = $DB->insert_record('block_analytics_config', $config);
        } else {
            $DB->update_record('block_analytics_config', $config);
        }
        $this->config = $config;
    }
     ...
     *
     * @param int $userid The user ID.
     * @param int $analytics The user XP.
     * @param int $lvl The known user level.
     * @return void
     */
    public function update_user_level($userid, $analytics = null, $lvl = null) {
        global $DB;

        if ($analytics === null || $lvl === null) {
            $record = $DB->get_record('block_analytics', array('courseid' => $this->courseid, 'userid' => $userid), 'analytics,lvl');
            if (!$record) {
                return;
            }
        ...
        $level = $this->get_level_from_analytics($analytics);
        if ($level != $lvl) {
            // Level up!
            $DB->set_field('block_analytics', 'lvl', $level, array('courseid' => $this->courseid, 'userid' => $userid));
            if ($this->triggereevents) {
                ...
                $event = \block_analytics\event\user_leveledup::create($params);
                $event->trigger();
            }
        }
