...
class block_analytics extends block_base {
    ...
    public function applicable_formats() {
        global $CFG;
        if (isset($CFG->block_analytics_context) && $CFG->block_analytics_context == CONTEXT_SYSTEM) {
    ...
    public function cron() {
        global $DB;
        $courseids = $DB->get_fieldset_sql('SELECT DISTINCT(courseid) FROM {block_analytics}', array());
    ...
    public function instance_create() {
        ...
        $manager = block_analytics_manager::get($this->page->course->id);
    ...
    public function get_content() {
        ...
        if (isset($this->config->description)) {
            ...
            $this->content->text .= $renderer->description(get_string('participatetolevelup', 'block_analytics'));
        }
        ...
        if ($canedit) {
            ...
            if (!$manager->get_config('enabled')) {
                $this->content->footer .= html_writer::tag('p',
                    html_writer::tag('small', get_string('analyticsgaindisabled', 'block_analytics')), array('class' => 'alert alert-warning'));
            }
        }
        ...
        if ($manager->has_levelled_up($USER->id)) {
            $args = array(
                ...
                'headline' => get_string('youreachedlevela', 'block_analytics', $progress->level),
                'level' => $progress->level,
            );

            $PAGE->requires->yui_module('moodle-block_analytics-notification', 'Y.M.block_analytics.Notification.init', array($args));
