...
$manager = block_analytics_manager::get($courseid);
...
// Some stuff.
$url = new moodle_url('/blocks/analytics/rules.php', array('courseid' => $courseid));
$strcourserules = get_string('courserules', 'block_analytics');
...
$renderer = $PAGE->get_renderer('block_analytics');
...
if (!empty($_POST['save'])) {
    ...
    foreach ($filters as $filterdata) {
        ...
        if (!block_analytics_filter::validate_data($data)) {
            throw new moodle_exception('Data could not be validated');
        }

        $filter = block_analytics_filter::load_from_data($data);
...
// Preparing form.
$dummyruleset = new block_analytics_ruleset();
$dummyfilter = block_analytics_filter::load_from_data(array('rule' => $dummyruleset));
$templatefilter = $renderer->render($dummyfilter);

// Templates of rules.
$typeruleproperty = new block_analytics_rule_property();
$typerulecm = new block_analytics_rule_cm($manager->get_courseid());
$typeruleevent = new block_analytics_rule_event();
$typeruleset = new block_analytics_ruleset();
$templatetypes = array(
    'block_analytics_rule_cm' => array(
        'name' => get_string('rulecm', 'block_analytics'),
        'template' => $renderer->render($typerulecm, array('iseditable' => true, 'basename' => 'XXXXX')),
    ),
    'block_analytics_rule_event' => array(
        'name' => get_string('ruleevent', 'block_analytics'),
        'template' => $renderer->render($typeruleevent, array('iseditable' => true, 'basename' => 'XXXXX')),
    ),
    'block_analytics_rule_property' => array(
        'name' => get_string('ruleproperty', 'block_analytics'),
        'template' => $renderer->render($typeruleproperty, array('iseditable' => true, 'basename' => 'XXXXX')),
    ),
    'block_analytics_ruleset' => array(
        'name' => get_string('ruleset', 'block_analytics'),
        'template' => $renderer->render($typeruleset, array('iseditable' => true, 'basename' => 'XXXXX')),
    ),
);

$PAGE->requires->yui_module('moodle-block_analytics-filters', 'Y.M.block_analytics.Filters.init', array(array(
    'filter' => $templatefilter,
    'rules' => $templatetypes
)));
$PAGE->requires->strings_for_js(array('pickaconditiontype'), 'block_analytics');
...
$a->log = (new moodle_url('/blocks/analytics/log.php', array('courseid' => $courseid)))->out();
$a->doc = (new moodle_url('https://docs.moodle.org/dev/Event_2'))->out();
echo get_string('rulesformhelp', 'block_analytics', $a);

echo html_writer::start_div('block-analytics-filters-wrapper');
echo html_writer::start_tag('form', array('method' => 'POST', 'class' => 'block-analytics-filters'));
echo html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'sesskey', 'value' => sesskey()));

$addlink = html_writer::start_tag('li', array('class' => 'filter-add'));
$addlink.= $renderer->action_link('#', get_string('addarule', 'block_analytics'), null, null,
    new pix_icon('t/add', '', '', array('class' => 'iconsmall')));
$addlink .= html_writer::end_tag('li');

echo $OUTPUT->heading(get_string('yourownrules', 'block_analytics'), 3);
...
echo $OUTPUT->heading(get_string('defaultrules', 'block_analytics'), 3);
echo html_writer::tag('p', get_string('defaultrulesformhelp', 'block_analytics'));

echo html_writer::start_tag('ul', array('class' => 'filters-list filters-readonly'));
foreach (block_analytics_filter_manager::get_static_filters() as $filter) {
    echo $renderer->render($filter);
