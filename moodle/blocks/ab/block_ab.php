 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_ab extends block_base {
    ...
    public function applicable_formats() {
        global $CFG;
        if (isset($CFG->block_ab_context) && $CFG->block_ab_context == CONTEXT_SYSTEM) {
    ...
    public function init() {
        $this->title = get_string('pluginname', 'block_ab');
    }
    ...
    //     $manager = block_ab_manager::get($this->page->course->id);
    ...
    public function get_content() {
       ...
       // $progress = $manager->get_progress_for_user($USER->id);
        $renderer = $this->page->get_renderer('block_ab');
