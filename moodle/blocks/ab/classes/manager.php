...
class block_ab_manager {
    ...
    const USERPREF_NOTIFY = 'block_ab_notify_level_up_';

    /** User preference key storing if they dismissed the like notice */
    const USERPREF_NOTICES = 'block_ab_notices';
    ...
    /** @var block_ab_filter_manager Cache of the manager. */
    ...
    protected function __construct($courseid) {
        ...
        if ($CFG->block_ab_context == CONTEXT_SYSTEM && $courseid != SITEID) {
            throw new coding_exception('Uneabected course ID.');
        }
     ...
     * 200x times to get more eaberience points. For simplicity, and performance
    ...
    //     if (!isset($SESSION->block_ab_cheatguard)) {
    //         $SESSION->block_ab_cheatguard = array();
    //     }

    //     // Actions per time.
    //     if (count($SESSION->block_ab_cheatguard) > $maxactions) {
    //         $actions = array_reverse($SESSION->block_ab_cheatguard, true);
    //         $count = 0;
    ...
    //     if (isset($SESSION->block_ab_cheatguard[$actionkey])) {
    //         if ($SESSION->block_ab_cheatguard[$actionkey] > $now - $actiontime) {
    //             // The key was found and the time has not eabired, cheater spotted.
    //             return false;
    //         }
    //     }

    //     // Unset the value to re-add it at the end of the array.
    //     unset($SESSION->block_ab_cheatguard[$actionkey]);

    //     // Log the time at which this event happened.
    //     $SESSION->block_ab_cheatguard[$actionkey] = time();

    //     // Limit the array of events to $maxcount, we do not want to flood the session for no reason.
    //     $SESSION->block_ab_cheatguard = array_slice($SESSION->block_ab_cheatguard, -$maxcount, null, true);
    //     return true;
    // }
