 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_ab_ladder_table extends table_sql {
    ...
    protected $identitymode = block_ab_manager::IDENTITY_ON;
    ...
    protected $rankmode = block_ab_manager::RANK_ON;
    ...
    protected $startingab;

    /** @var int The XP to compare with. Used with RANK_REL. */
    protected $startingabdiff;

    /** @var array The fields found in the XP table. */
    public static $abfields = array('id', 'courseid', 'userid', 'ab', 'lvl');
    ...
    public function __construct($uniqueid, $courseid, $groupid, array $options = array(), $userid = null) {
        ...
        $columns = array();
        $headers = array();
        if ($this->rankmode != block_ab_manager::RANK_OFF) {
            $columns += array('rank');
            if ($this->rankmode == block_ab_manager::RANK_REL) {
                $headers += array(get_string('difference', 'block_ab'));
            } else {
                $headers += array(get_string('rank', 'block_ab'));
            }
        }
        ...
        if ($groupid) {
            ...
            $sqlfrom = '{block_ab} x JOIN {user} u ON x.userid = u.id';
        }
    ...
    function build_table() {
        ...
        $lastab = $this->startingab;
        $offset = $this->startingoffset;
        $abtodiff = $this->startingabdiff;
        ...
        if ($this->rawdata) {
            foreach ($this->rawdata as $row) {
                ...
                // Show the real rank.
                if ($this->rankmode == block_ab_manager::RANK_ON) {

                    // If this row is different than the previous one.
                    if ($row->lvl != $lastlvl || $row->ab != $lastab) {
                        $rank += $offset;
                        $offset = 1;
                        $lastlvl = $row->lvl;
                        $lastab = $row->ab;
    ...
    public function col_fullname($row) {
        if ($this->identitymode == block_ab_manager::IDENTITY_OFF && $row->userid != $this->userid) {
            return get_string('someoneelse', 'block_ab');
        }
    ...
    protected function col_progress($row) {
        static $fields = null;
        if ($fields === null) {
            $fields = array_flip(self::$abfields);
        }

        $record = (object) array_intersect_key((array) $row, $fields);
        $progress = $this->abmanager->get_progress_for_user($row->userid, $record);
    ...
    protected function col_rank($row) {
        if ($this->rankmode == block_ab_manager::RANK_REL && $row->rank > 0) {
            return '+' . $row->rank;
        }
        return $row->rank;
    }
    ...
    protected function col_userpic($row) {
        ...
        if ($this->identitymode == block_ab_manager::IDENTITY_OFF && $this->userid != $row->userid) {
    ...
    protected function compute_rank_start() {
        ...
        $this->startingabdiff = -1;

        // Guess the starting rank.
        if ($this->rankmode == block_ab_manager::RANK_ON && !empty($this->rawdata)) {
            $record = reset($this->rawdata);
            $sql = "SELECT COUNT(x.id)
                      FROM {$this->sql->from}
                     WHERE {$this->sql->where}
                       AND x.ab > :neighab";
            $this->startingrank = $DB->count_records_sql($sql, $this->sql->params + array('neighab' => $record->ab)) + 1;
    ...
    protected function get_user_record() {
        ...
        if ($this->currentuserrecord === null) {
            ...
            if (empty($record) && $this->neighboursonly && $this->abmanager->can_manage()) {
                $record = (object) array(
                    'id' => 0,
                    'userid' => $this->userid,
    ...
    public function get_sort_columns() {
        return array('x.lvl' => SORT_DESC, 'x.ab' => SORT_DESC, 'x.id' => SORT_ASC);
    }
        ...
        if (!$this->is_downloading()) {
            ...
            if ($requestedpage === null && ($record = $this->get_user_record())) {
                $sql = "SELECT COUNT('x')
                         ...
                         WHERE {$this->sql->where}
                           AND (x.ab > :theab
                            OR (x.ab = :theabeq AND x.id < :theid))";
                $params = $this->sql->params + array(
                    'theab' => $record->ab,
                    'theabeq' => $record->ab,
                    'theid' => $record->id
    ...
    public function query_db_neighbours($userid, $abovecount, $belowcount) {
        ...
        $sqlabove = "SELECT {$this->sql->fields}
                      ...
                      WHERE {$this->sql->where}
                        AND (x.ab > :neighab
                         OR (x.ab = :neighabeq AND x.id < :neighid))
                      ORDER BY x.ab ASC, x.id DESC";
        $sqlbelow = "SELECT {$this->sql->fields}
                       FROM {$this->sql->from}
                      WHERE {$this->sql->where}
                        AND (x.ab < :neighab
                         OR (x.ab = :neighabeq AND x.id > :neighid))
                      ORDER BY x.ab DESC, x.id ASC";
