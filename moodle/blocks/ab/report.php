...
// Some stuff.
$url = new moodle_url('/blocks/ab/report.php', array('courseid' => $courseid));
if ($action) {
    $url->param('action', $action);
}
$strcoursereport = get_string('coursereport', 'block_ab');
...
// Some other stuff.
$renderer = $PAGE->get_renderer('block_ab');
...
if ($resetdata && confirm_sesskey()) {
    if ($confirm) {
        ...
        $resetstr = empty($group) ? get_string('reallyresetdata', 'block_ab') : get_string('reallyresetgroupdata', 'block_ab');
...
$table = new block_ab_report_table('block_ab_report', $courseid, $group);
