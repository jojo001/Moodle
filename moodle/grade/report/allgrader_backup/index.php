...
require_once $CFG->dirroot.'/grade/report/allgrader/lib.php';
require_once $CFG->dirroot.'/grade/report/allgrader/locallib.php'; // END OF HACK
//require_once $CFG->dirroot.'/grade/report/allgrader/all_grade_export_xls.php';
...
$itemid        = optional_param('itemid', 0, PARAM_ALPHANUM); // item to zerofill or clear overrides -- allgrader
...
$PAGE->set_url(new moodle_url('/grade/report/allgrader/index.php', array('id'=>$courseid)));
...
require_capability('gradereport/allgrader:view', $context);
require_capability('moodle/grade:viewall', $context);

/// return tracking object
$gpr = new grade_plugin_return(array('type'=>'report', 'plugin'=>'allgrader', 'courseid'=>$courseid, 'page'=>$page));
...
$USER->grade_last_report[$course->id] = 'allgrader';
...
$reportname = get_string('pluginname', 'gradereport_allgrader');

/// Print header
if ($action !== 'quick-dump') {
    print_grade_page_head($COURSE->id, 'report', 'allgrader', $reportname, false, $buttons);
}

// Initialise the grader report object
$report = new grade_report_allgrader($courseid, $gpr, $context, $page, $sortitemid); // END OF HACK
...
//$reporthtml .= '<script src="jquery-1.7.2.min.js" type="text/javascript"></script>';
		...
		$scrolling = get_user_preferences('grade_report_allgrader_reportheight');
		$scrolling = $scrolling == null ? 300 + ($CFG->grade_report_allgrader_reportheight * 40) : 300 + ($scrolling * 40);
        ...
        $headerinit = "fxheaderInit('all-user-grades', $scrolling," . $headerrows . ',' . $headercols . ');';
		$reporthtml .=
		        '<script src="' . $CFG->wwwroot . '/grade/report/allgrader/fxHeader_0.6.min.js" type="text/javascript"></script>
		         <script type="text/javascript">' .$headerinit . 'fxheader(); </script>';

		// $reporthtml .=
		//         '<script src="' . $CFG->wwwroot . '/grade/report/allgrader/my_jslib.js" type="text/javascript"></script>';
...
//     echo '<input type="hidden" value="allgrader" name="report"/>';
...
// print submit button
if ($report->get_pref('showquickfeedback') || $report->get_pref('quickgrading')){
    echo '<form action="index.php" method="post">';
    echo '<div>';
    echo '<input type="hidden" value="'.s($courseid).'" name="id" />';
    echo '<input type="hidden" value="'.sesskey().'" name="sesskey" />';
