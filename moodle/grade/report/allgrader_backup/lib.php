...
require_once $CFG->dirroot.'/grade/report/allgrader/locallib.php';
 ...
 * DONE
 */
class grade_report_allgrader extends grade_report_grader {
    /**
     * The final grades.
     * @var array $grades
     */
...
//// SQL-RELATED
    ...
    function __construct($courseid, $gpr, $context, $page=null, $sortitemid=null) {
       ...
       // $this->accuratetotals		= ($temp = grade_get_setting($this->courseid, 'report_allgrader_accuratetotals', $CFG->grade_report_allgrader_accuratetotals)) ? $temp : 0;
        ...
        $this->columnwidth = get_user_preferences('grade_report_allgrader_columnwidth');
		$this->columnwidth = $this->columnwidth == null ? 25 + ($CFG->grade_report_allgrader_columnwidth * 5) : 25 + ($this->columnwidth * 5);

        // need this array, even tho its useless in the allgrader report or we'll generate warnings
        ...
        // Fill items with parent information needed later for allgrader report
      ...
      //   }
        ...
        $this->pbarurl = new moodle_url('/grade/report/allgrader/index.php', array('id' => $this->courseid, 'perpage' => $studentsperpage));
    ...
    public function get_left_rows() {
        ...
        $adminextrafields = $CFG->grade_report_allgrader_extrafields == 1 ? 1 : 0 ;
        $userextrafields = get_user_preferences( 'grade_report_allgrader_extrafields' );
		...
		$output = '<div class="inlinebutton" title="' . get_string('copytoexcel', 'gradereport_allgrader') . '">';
        $output .= '<a href="' . $CFG->wwwroot . '/grade/report/allgrader/index.php?id=' . $this->courseid
                . '&action=quick-dump" class="inlinebutton"><img src="' . $CFG->wwwroot . '/grade/report/allgrader/images/copytoexcel.png" /></a></div>';
...
//			$gradetabindex++;
    ...
    public function get_grade_table() {
        global $OUTPUT;
 		$fixedstudents = 0; // always for allgrader report
//        $fixedstudents = $this->is_fixed_students();
        ...
        $fulltable->attributes['class'] = 'allgradestable';
        $fulltable->id = 'all-user-grades';
...
//     }
    ...
    // 	if (!isset($this) OR get_class($this) != 'grade_report_allgrader') {
    // 		if (!empty($objectid)) {
...
// function grade_report_allgrader_settings_definition(&$mform) {
// 	global $CFG;

// 	$options = array(-1 => get_string('default', 'grades'),
// 			0 => get_string('hide'),
// 			1 => get_string('show'));
// 	if (empty($CFG->grade_report_allgrader_accuratetotals)) {
// 		$options[-1] = get_string('defaultprev', 'grades', $options[0]);
// 	} else {
// 		$options[-1] = get_string('defaultprev', 'grades', $options[1]);
// 	}
// 	$mform->addElement('select', 'report_allgrader_accuratetotals', get_string('accuratetotals', 'gradereport_allgrader'), $options);
// 	$options = array(-1 => get_string('default', 'grades'),
// 			0 => '300',
// 			1 => '340',
// 			2 => '380',
...
// 			14 => '860',
// 			15 => '900');
// 	if (empty($CFG->grade_report_allgrader_reportheight)) {
// 		$options[-1] = get_string('defaultprev', 'grades', $options[8]);
// //	} else {
// //		$options[-1] = get_string('defaultprev', 'grades', $options[1]);
// 	}
// 	$mform->addElement('select', 'report_allgrader_reportheight', get_string('allgrader_reportheight', 'gradereport_allgrader'), $options);
// 	$options = array(-1 => get_string('default', 'grades'),
// 			0 => '25',
// 			1 => '30',
// 			2 => '35',
...
// 			12 => '85',
// 			13 => '90');
// 	if (empty($CFG->grade_report_allgrader_columnwidth)) {
// 		$options[-1] = get_string('defaultprev', 'grades', $options[0]);
// //	} else {
// //		$options[-1] = get_string('defaultprev', 'grades', $options[1]);
// 	}
// 	$mform->addElement('select', 'report_allgrader_columnwidth', get_string('allgrader_columnwidth', 'gradereport_allgrader'), $options);
// }
