 ...
 */
class allgrader_report_preferences_form extends moodleform {
...
//                $preferences['prefshow']['showaverages']  = $checkbox_default;
        ...
        if (has_capability('gradereport/allgrader:view', $context)) {
            $preferences['prefshow']['studentsperpage'] = 'text'; //no students per page in Maths grader report
...
//            $preferences['prefshow']['showzerofill'] = $checkbox_default;
        ...
        if (has_capability('moodle/grade:edit', $context)) {
            ...
            $preferences['prefshow']['allgrader_reportheight'] = array(300,340,380,420,460,500,540,580,620,660,700,740,780,820,860,900);
            $preferences['prefshow']['allgrader_columnwidth'] = array(25,30,35,40,45,50,55,60,65,70,75,80,85);
            $ef_default = $CFG->grade_report_allgrader_extrafields == 1 ? get_string('yes') : get_string('no') ;
            $preferences['prefshow']['allgrader_extrafields'] = array(GRADE_REPORT_PREFERENCE_DEFAULT => get_string('reportdefault', 'grades', $ef_default), 0 => get_string('no'), 1 => get_string('yes'));
        }
        ...
        foreach ($preferences as $group => $prefs) {
            ...
            foreach ($prefs as $pref => $type) {
                ...
                if (is_array($type)) {
                    ...
                    } else if ($full_pref == 'grade_report_allgrader_reportheight') {
                ...
                if ($lang_string == 'showclearoverrides' || $lang_string == 'allgrader_reportheight' || $lang_string == 'allgrader_columnwidth' || $lang_string == 'showzerofill' || $lang_string == 'allgrader_extrafields') {
                	$label = get_string($lang_string, 'gradereport_allgrader') ;
                } else {
                	$label = get_string($lang_string, 'grades') . $number;
                }

                $mform->addElement($type, $full_pref, $label, $options);
                if ($lang_string == 'allgrader_extrafields') {
                    $mform->addHelpButton($full_pref, $lang_string, 'gradereport_allgrader');
                }
                elseif ($lang_string != 'showuserimage' && $lang_string != 'showclearoverrides' && $lang_string != 'allgrader_reportheight' && $lang_string != 'allgrader_columnwidth' && $lang_string != 'showzerofill') {
