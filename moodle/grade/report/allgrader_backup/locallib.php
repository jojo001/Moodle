...
/*        if (array_key_exists($element['object']->id, $this->modx)) { // don't include something made only for a different group
     ...
     * USED BY allgrader IN ORDER TO WRAP GRADE TITLES IN THE HEADER
     *
    ...
    function get_element_header_local(&$element, $withlink=false, $icon=true, $spacerifnone=false, $titlelength = null, $catname) {
	    ...
	    switch ($element['type']) {
			case 'courseitem':
				$header .= get_string('coursetotal', 'gradereport_allgrader');
				break;
			case 'categoryitem':
				$header .= get_string('categorytotal', 'gradereport_allgrader');
			default:
		 		$header .= $catname . ' ';
		}
...
// 	}
  ...
  //       $strzerofill = get_string('zerofill', 'gradereport_allgrader', $strparams);

  //       $url = new moodle_url('/grade/report/allgrader/index.php', array('id' => $this->courseid, 'sesskey' => sesskey(), 'action' => 'quickdump'));
  //       $url = $gpr->add_url_params($url);

  //       $type = 'zerofill';
  //       $tooltip = $strzerofill;
  //       $actiontext = '<img alt="' . $type . '" class="smallicon" title="' . $strzerofill . '" src="' . $CFG->wwwroot . '/grade/report/allgrader/images/zerofill.png" />';
  ...
  //   }
    ...
    //     $strparams = $this->get_params_for_iconstr($element);
    //     $strclearoverrides = get_string('clearoverrides', 'gradereport_allgrader', $strparams);

    //     if ($element['type'] == 'item') {
    //         $url = new moodle_url('/grade/report/allgrader/index.php', array('id' => $this->courseid, 'sesskey' => sesskey(), 'action' => 'clearoverrides', 'itemid'=>$element['object']->id));
    //     } else {
    //         $element['object']->load_grade_item();
    //     	$url = new moodle_url('/grade/report/allgrader/index.php', array('id' => $this->courseid, 'sesskey' => sesskey(), 'action' => 'clearoverrides', 'itemid'=>$element['object']->grade_item->id));
    //     }

    //     $type = 'clearoverrides';
    //     $tooltip = $strclearoverrides;
    //     $actiontext = '<img alt="' . $type . '" class="smallicon" title="' . $strclearoverrides . '" src="' . $CFG->wwwroot . '/grade/report/allgrader/images/clearoverrides.gif" />';
    ...
    public function get_changedisplay_icon($element) {
        ...
        $strparams = $this->get_params_for_iconstr($element);
        $strchangedisplay = get_string('changedisplay', 'gradereport_allgrader', $strparams);

        $url = new moodle_url('/grade/report/allgrader/index.php', array('id' => $this->courseid, 'sesskey' => sesskey(), 'action' => 'changedisplay', 'itemid'=>$element['object']->id));

        $type = 'changedisplay';
        $tooltip = $strchangedisplay;
		$actiontext = '<img alt="' . $type . '" title="' . $strchangedisplay . '" src="' . $CFG->wwwroot . '/grade/report/allgrader/images/changedisplay.png" />';
