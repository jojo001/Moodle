...
require_once $CFG->dirroot.'/grade/report/mygrader/locallib.php';
 ...
 * DONE
 */
class grade_report_mygrader extends grade_report_grader {
    /**
     * The final grades.
     * @var array $grades
     */
...
//// SQL-RELATED
    ...
    function __construct($courseid, $gpr, $context, $page=null, $sortitemid=null) {
        ...
        $this->accuratetotals		= ($temp = grade_get_setting($this->courseid, 'report_mygrader_accuratetotals', $CFG->grade_report_mygrader_accuratetotals)) ? $temp : 0;
        ...
        $this->columnwidth = get_user_preferences('grade_report_mygrader_columnwidth');
		$this->columnwidth = $this->columnwidth == null ? 25 + ($CFG->grade_report_mygrader_columnwidth * 5) : 25 + ($this->columnwidth * 5);

        // need this array, even tho its useless in the mygrader report or we'll generate warnings
      ...
      // Fill items with parent information needed later for mygrader report           //IMPORTANT
    ...
    $this->gtree->cats = array();
        ...
        $this->pbarurl = new moodle_url('/grade/report/mygrader/index.php', array('id' => $this->courseid, 'perpage' => $studentsperpage));
  ...
  //  	}
    ...
    public function get_left_rows() {
        ...
        $adminextrafields = $CFG->grade_report_mygrader_extrafields == 1 ? 1 : 0 ;
        $userextrafields = get_user_preferences( 'grade_report_mygrader_extrafields' );
		...
		$output = '<div class="inlinebutton" title="' . get_string('copytoexcel', 'gradereport_mygrader') . '">';
        $output .= '<a href="' . $CFG->wwwroot . '/grade/report/mygrader/index.php?id=' . $this->courseid
                . '&action=quick-dump" class="inlinebutton"><img src="' . $CFG->wwwroot . '/grade/report/mygrader/images/copytoexcel.png" /></a></div>';
...
//			$gradetabindex++;
    ...
    public function get_grade_table() {
        global $OUTPUT;
 		$fixedstudents = 0; // always for mygrader report
...
//     	if (!isset($this) OR get_class($this) != 'grade_report_mygrader') {
//     		if (!empty($objectid)) {
...
// function grade_report_mygrader_settings_definition(&$mform) {
// 	global $CFG;

// 	$options = array(-1 => get_string('default', 'grades'),
// 			0 => get_string('hide'),
// 			1 => get_string('show'));
// 	if (empty($CFG->grade_report_mygrader_accuratetotals)) {
// 		$options[-1] = get_string('defaultprev', 'grades', $options[0]);
// 	} else {
// 		$options[-1] = get_string('defaultprev', 'grades', $options[1]);
// 	}
// 	$mform->addElement('select', 'report_mygrader_accuratetotals', get_string('accuratetotals', 'gradereport_mygrader'), $options);
// 	$options = array(-1 => get_string('default', 'grades'),
// 			0 => '300',
// 			1 => '340',
// 			2 => '380',
...
// 			14 => '860',
// 			15 => '900');
// 	if (empty($CFG->grade_report_mygrader_reportheight)) {
// 		$options[-1] = get_string('defaultprev', 'grades', $options[8]);
// //	} else {
// //		$options[-1] = get_string('defaultprev', 'grades', $options[1]);
// 	}
// 	$mform->addElement('select', 'report_mygrader_reportheight', get_string('mygrader_reportheight', 'gradereport_mygrader'), $options);
// 	$options = array(-1 => get_string('default', 'grades'),
// 			0 => '25',
// 			1 => '30',
// 			2 => '35',
...
// 			12 => '85',
// 			13 => '90');
// 	if (empty($CFG->grade_report_mygrader_columnwidth)) {
// 		$options[-1] = get_string('defaultprev', 'grades', $options[0]);
// //	} else {
// //		$options[-1] = get_string('defaultprev', 'grades', $options[1]);
// 	}
// 	$mform->addElement('select', 'report_mygrader_columnwidth', get_string('mygrader_columnwidth', 'gradereport_mygrader'), $options);
// }
