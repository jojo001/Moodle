...
require_once $CFG->dirroot.'/grade/report/mygrader/lib.php';
require_once $CFG->dirroot.'/grade/report/mygrader/locallib.php'; // END OF HACK
//require_once $CFG->dirroot.'/grade/report/mygrader/lae_grade_export_xls.php';
...
$itemid        = optional_param('itemid', 0, PARAM_ALPHANUM); // item to zerofill or clear overrides -- mygrader
...
$PAGE->set_url(new moodle_url('/grade/report/mygrader/index.php', array('id'=>$courseid)));
...
require_capability('gradereport/mygrader:view', $context);
require_capability('moodle/grade:viewall', $context);

/// return tracking object
$gpr = new grade_plugin_return(array('type'=>'report', 'plugin'=>'mygrader', 'courseid'=>$courseid, 'page'=>$page));
...
// if ($action == 'changedisplay' && $itemid !==0) {
...
$USER->grade_last_report[$course->id] = 'mygrader';
...
$reportname = get_string('pluginname', 'gradereport_mygrader');

/// Print header
if ($action !== 'quick-dump') {
    print_grade_page_head($COURSE->id, 'report', 'mygrader', $reportname, false, $buttons);
}

// Initialise the grader report object
$report = new grade_report_mygrader($courseid, $gpr, $context, $page, $sortitemid); // END OF HACK
...
$reporthtml .= '<script src="jquery-1.7.2.min.js" type="text/javascript"></script>';
		...
		$scrolling = get_user_preferences('grade_report_mygrader_reportheight');
		$scrolling = $scrolling == null ? 300 + ($CFG->grade_report_mygrader_reportheight * 40) : 300 + ($scrolling * 40);
		...
		$reporthtml .=
		        '<script src="' . $CFG->wwwroot . '/grade/report/mygrader/fxHeader_0.6.min.js" type="text/javascript"></script>
		        <script type="text/javascript">' .$headerinit . 'fxheader(); </script>';

		$reporthtml .=
		        '<script src="' . $CFG->wwwroot . '/grade/report/mygrader/my_jslib.js" type="text/javascript"></script>';
...
// }
  echo '<form action="index.php" method="post">';
    ...
    echo '<input type="hidden" value="mygrader" name="report"/>';
